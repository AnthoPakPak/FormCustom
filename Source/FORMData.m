...
-(NSMutableArray*)allGroupsIncludingHiddenOnes {
    if (_allGroupsIncludingHiddenOnes) return _allGroupsIncludingHiddenOnes;
    
    _allGroupsIncludingHiddenOnes = [NSMutableArray new];
    return _allGroupsIncludingHiddenOnes;
}
...
- (void)generateFormsWithJSON:(id)JSON
    ...
    self.allGroupsIncludingHiddenOnes = [self.groups copy]; //ANTHO
                ...
                if (field) {
                    ...
                    for (FORMSection *section in group.sections) {
                        if ([section.sectionID isEqualToString:field.section.sectionID]) {
                            ...
                            //IMPLEM BY ANTHO POUR CORRIGER LORDRE QUAND ON AFFICHE
                            NSArray *allFieldIDsInGroup = [self fieldIDsFromJSONForGroupIndex:[field.section.group.position integerValue]];
                            NSArray *visibleFieldIDsInSection = [section.fields valueForKey:@"fieldID"]; //cette implem ne fonctionne que quand on n'a qu'une seule section !!!
                            if ([allFieldIDsInGroup containsObject:field.fieldID]) {
                                fieldIndex = [allFieldIDsInGroup indexOfObject:field.fieldID];
                                
                                for (NSString *fieldIdHiddenOrNot in allFieldIDsInGroup) {
                                    if (![visibleFieldIDsInSection containsObject:fieldIdHiddenOrNot]) { //element caché
                                        if ([allFieldIDsInGroup indexOfObject:fieldIdHiddenOrNot] < [allFieldIDsInGroup indexOfObject:field.fieldID])
                                            fieldIndex--;
                                    }
                                }
                            }
...
- (NSArray *)fieldIDsFromJSONForGroupIndex:(NSInteger)sectionIndex {
    NSMutableArray *fieldIDs = [[_JSON valueForKeyPath:@"groups.sections.fields.id"] mutableCopy];
    [fieldIDs removeObjectIdenticalTo:[NSNull null]];
    
    return [self flatten:fieldIDs[sectionIndex]];
}
