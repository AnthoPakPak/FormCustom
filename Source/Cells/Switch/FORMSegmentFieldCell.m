...
//      if (selectedIndex && selectedIndex != UISegmentedControlNoSegment) {
    ...
    NSLog(@"Segment %p restored value : %d | selectedIndex var : %d", self, self.segment.selectedSegmentIndex, selectedIndex);
...
- (void)segmentAction:(id)sender {
    NSLog(@"Segment clicked on value : %d", self.segment.selectedSegmentIndex);
    
    FORMFieldValue *value = [self.field.values objectAtIndex:[sender selectedSegmentIndex]];
    self.field.value = value;
    
    selectedIndex = [self.segment selectedSegmentIndex]; //ANTHO
...
-(void) manuallySetSegmentValueToIndex:(NSInteger)index {
    selectedIndex = index;
    [self.segment setSelectedSegmentIndex:index];
}
