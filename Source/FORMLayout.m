...
- (UICollectionViewLayoutAttributes *)layoutAttributesForDecorationViewOfKind:(NSString *)elementKind
    ...
    if (![elementKind isEqualToString:viewOfKind]) {
        ...
        if (fields.count > 0) {
//            NSArray<NSIndexPath *> *visibleItemsIndexPaths = self.collectionView.indexPathsForVisibleItems;
////            NSMutableArray<NSIndexPath *> *visibleItemsInSectionIndexPaths = @[].mutableCopy;
////
////            for (NSIndexPath *visibleItemIndexPath in visibleItemsIndexPaths) {
////                if (visibleItemIndexPath.section == indexPath.section) {
////                    [visibleItemsInSectionIndexPaths addObject:visibleItemIndexPath];
////                }
////            }
////
//            NSIndexPath *lastVisibleIndexPath = [visibleItemsInSectionIndexPaths lastObject];


            NSIndexPath *lastIndexPath = [NSIndexPath indexPathForItem:fields.count - 1
                                                             inSection:indexPath.section];
            
            @try { //ANTHO fix crash quand on clique sur le segmente (avec target show), alors qu'on a déjà un peu scroll le form
            ...
            @catch (NSException *exception) {
                NSLog(@"exce : %@", exception.reason);
                NSLog(@"Crash evited !");
                lastIndexPath = [NSIndexPath indexPathForItem:lastIndexPath.row - 1
        ...
        if (indexPath.section > 0 && height > 0) {
            ...
            if (previousFields.count > 0) {
                ...
                @catch (NSException *exception) {
                    ...
                    previousIndexPath = [NSIndexPath indexPathForItem:previousIndexPath.row - 1
                                                            inSection:previousIndexPath.section];
